...
**Brand alignment and native QA pass — COMPLETE.** This controlled pass sits between Milestones 2 and 3. It aligned the static shell to the earlier TEKRARLA identity, introduced licensed brand typography with fallback, and verified the Expo Go workflow on an iOS simulator. The app name is now settled as **Tekrarla**; `tekrarla` is the technical identifier in slug, scheme, bundle ID, and package name. It adds no product or learning logic.
